...
# Update requirement
if rg -q "github.com/otterlab-bio/bamdriver" go.mod; then
  go mod edit -require=github.com/otterlab-bio/bamdriver@"$VERSION"
else
  go mod edit -require=github.com/otterlab-bio/bamdriver@"$VERSION"
fi

# Remove local replace if present
if rg -q "replace github.com/otterlab-bio/bamdriver" go.mod; then
  go mod edit -dropreplace=github.com/otterlab-bio/bamdriver || true
fi
